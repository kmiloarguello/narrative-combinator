...
def export_dashboard(items: list[StoryQuality], language: str, path: Path) -> None:
    ...
    rows = ''.join(f"<tr><td>{i.story_id}</td><td>{i.score}</td><td>{i.word_count}</td><td>{i.sentence_count}</td><td>{escape(', '.join(i.repeated_keywords) or '—')}</td><td>{i.review_priority}</td></tr>" for i in items)
